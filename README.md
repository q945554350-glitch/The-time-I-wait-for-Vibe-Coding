...
然后访问 <http://127.0.0.1:8000>。这种方式会自动监听本机新完成的 Codex 任务；任务结束时，游戏会暂停并弹出提醒。

直接双击 `index.html` 仍然可以游玩，但不会启用任务监听。也可以使用普通静态服务器进入纯游戏模式：
...
其他编辑器、脚本或代码代理完成任务后，可以调用本地通知命令：

```bash
python3 notify_task.py "Code"
```

也可以附带一条简短说明：

```bash
python3 notify_task.py "Code" --summary "构建和测试已经完成"
```

监听只在 `127.0.0.1` 本机运行。Codex 监听器只识别本机新增的任务完成事件，不会把提示词、代码或任务内容发送到游戏页面或外部网络。
...
- 连续解决问题会累积 Combo，提高射速并增加 Token 掉落；受伤或中断连杀会清空。
- 敌人和 Boss 的耐久会随等待时间提高；后期会出现远程弹幕、连锁自爆、精英问题和周期性增援潮。
- 危险贴近时闪身会触发「漂亮撤回」，短暂减速战场并提高暴击率。
- 特定模型增幅可以形成联动，例如上下文暴击、提示词洪流和咖啡自动续杯。
- 每局总共可以免费刷新两次「模型增幅」选项，也可以按 `R` 快速刷新。
- 内置 12 个战斗、操作、构筑与挑战成就，解锁记录会保存在当前浏览器中。
